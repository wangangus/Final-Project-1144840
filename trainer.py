...
class Trainer(object):
    def __init__(self,
                 nnet,
                 checkpoint="/media/kaldi/SP1/MingHshuan/checkpoint",
                 optimizer="adam",
                 gpuid=0,
        ...
        if resume:
            ...
            self.optimizer = self.create_optimizer(
                optimizer, optimizer_kwargs, state=cpt["weightoptim_state_dict"])
        else:
            self.nnet = nnet.to(self.device)
            # fliter
            self.trainable_params = [p for p in self.nnet.parameters() if p.requires_grad]
    ...
    def train(self, data_loader):
        ...
        self.nnet.train()
        quantized_model = th.quantization.quantize_dynamic(self.nnet, {th.nn.Linear}, dtype=th.qint8)
...
def plot(train_loss_lst, dev_loss_lst, num_epochs):
    ...
    plt.plot(range(1, len(train_loss_lst) + 1), train_loss_lst, label='Train', color='red')
    plt.plot(range(1, len(train_loss_lst) + 1), dev_loss_lst, label='Dev', color='blue')

...
class MS_SL2_split_model(nn.Module):
    def __init__(self,
        ...
        # ★新增方向 B 的 Temporal Gate Layer (1x1-Conv)
        # 保持通道數不變 (Sc -> Sc)，且 kernel_size=1
        self.gate_conv = nn.Conv1d(Sc, Sc, 1)
    ...
    def forward(self, x):
        ...
        for Slice in range(self.slice):
            ...
            # 1. 【修正 Cyclic】利用 th.roll 讓權重隨 Slice 循環平移
            shifted_wList = th.roll(self.wList, shifts=Slice)
            
            # 2. 【方向 B】透過 1x1-Conv + Sigmoid 算出時間軸的閘門 g
            g = th.sigmoid(self.gate_conv(skip_connection))
            
            # 3. 【方向 B】進行三重相乘 (h ⊙ g ⊙ w)
            # 先將原特徵與閘門逐元素相乘 (時間軸選擇性保留)
            gated_skip = skip_connection * g
            
            # 再乘上平移後的 Cyclic 權重
            slic_into_weight = th.einsum('i,jkl->ijkl', [shifted_wList, gated_skip])
            slice_row_sum = th.einsum('ijkl->jkl', [slic_into_weight])
